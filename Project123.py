...
def handler():
    ...
    try:
        # убедитесь, что мы ввели правильные значения
        a_val = float(a.get())
        b_val = float(b.get())
        c_val = float(c.get())
...
root.title("ПО для решения квадратных уравнений(pre-alpha)")
...
# иконка приложения
root.iconbitmap('C:/Users/user/Desktop/x2.ico')
...
output = Text(frame, bg="White", font="Сolibri 14", width=35, height=10)
